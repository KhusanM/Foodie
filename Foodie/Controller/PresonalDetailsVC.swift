...
class PresonalDetailsVC: UIViewController{
    ...
    override func viewDidLoad() {
        ...
        txtName.text = dat?.name
        txtEmail.text = dat?.email
        txtPhoneNumber.text = dat?.phoneNumber
        //txtViewAddress.text = dat?.address
    ...
    // MARK: ENCODER DATA
    func encoder(){
       let encoders = JSONEncoder()
       if let encoderData = try? encoders.encode(dat) {
           UserDefaults.standard.set(encoderData, forKey: "data")
       }
   }
